...
import type { LaunchCommand, HAREntry, HAR } from './types.js';
...
export class BrowserManager {
  ...
  // Full HAR recording (request + response bodies)
  private harEntries: HAREntry[] = [];
  private pendingRequests: Map<string, { entry: HAREntry; startTime: number }> = new Map();
  ...
  /**
   * Start full HAR recording with request/response bodies
   */
  async startHarRecording(): Promise<void> {
    if (this.isRecordingHar) return;
    ...
    page.on('request', (request: Request) => {
      ...
      const entry: HAREntry = {
        ...
        request: {
          ...
          queryString,
          postData,
          bodySize: postDataText ? postDataText.length : 0,
          headersSize: -1,
          cookies: [],
        },
        response: {
      ...
      const requestId = `${request.method()}-${url}-${startTime}`;
      this.pendingRequests.set(requestId, { entry, startTime });
    });
    ...
    page.on('response', async (response: PlaywrightResponse) => {
      ...
      let matchedEntry: HAREntry | null = null;
      let matchedKey: string | null = null;
      let matchedStartTime = 0;

      for (const [key, value] of this.pendingRequests.entries()) {
        if (key.includes(url)) {
          matchedEntry = value.entry;
          matchedKey = key;
          matchedStartTime = value.startTime;
          break;
        }
      }

      if (!matchedEntry || !matchedKey) {
        // No matching request found, create a new entry
      ...
      matchedEntry.response.status = response.status();
      matchedEntry.response.statusText = response.statusText();
      matchedEntry.response.headers = Object.entries(response.headers()).map(
        ([name, value]) => ({ name, value })
      );
      matchedEntry.response.content.mimeType =
      ...
      try {
        ...
        if (body !== null) {
          matchedEntry.response.content.text = body;
          matchedEntry.response.content.size = body.length;
          matchedEntry.response.bodySize = body.length;
        }
      } catch {
        // Body not available
      }
      ...
      // Calculate timing
      const totalTime = endTime - matchedStartTime;
      matchedEntry.time = totalTime;
      matchedEntry.timings.receive = totalTime;

      // Add to entries
      this.harEntries.push(matchedEntry);
    });
  }
  ...
  /**
   * Export recorded HAR as JSON string
   */
  exportHar(): string {
    const har: HAR = {
      log: {
