...
export async function executeCommand(command: Command, browser: BrowserManager): Promise<Response> {
  try {
    switch (command.action) {
      ...
      case 'har_export':
        return await handleHarExport(command, browser);
      case 'har_clear':
        return handleHarClear(browser);
      case 'state_save':
...
async function handleHarStart(
  ...
  browser.startRequestTracking();
  return successResponse(command.id, { started: true, recording: true });
}

async function handleHarStop(command: HarStopCommand, browser: BrowserManager): Promise<Response> {
  browser.stopHarRecording();
  const entryCount = browser.getHarEntryCount();

  // Optionally save to file if path provided
  if (command.path) {
    const fs = await import('node:fs');
    const harContent = browser.exportHar();
    fs.writeFileSync(command.path, harContent);
  }
...
async function handleHarExport(
  command: Command & { action: 'har_export' },
  browser: BrowserManager
): Promise<Response> {
  const har = browser.exportHar();
  const entryCount = browser.getHarEntryCount();
  return successResponse(command.id, {
    har,
    entryCount,
...
function handleHarClear(browser: BrowserManager): Response {
  browser.clearHar();
