...
export interface HarStopCommand extends BaseCommand {
  action: 'har_stop';
  path?: string; // Optional path to save HAR file
}

export interface HarExportCommand extends BaseCommand {
  action: 'har_export';
}

export interface HarClearCommand extends BaseCommand {
  action: 'har_clear';
}

// Full HAR 1.2 entry for recording
export interface HAREntry {
  ...
  timings: { wait: number; receive: number; send: number; blocked: number; dns: number; connect: number; ssl: number };
}
