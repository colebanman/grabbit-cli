...
pub fn parse_command(args: &[String], flags: &Flags) -> Result<Value, ParseError> {
    ...
    match cmd {
        ...
        // === HAR Recording ===
        "har" => {
            const VALID: &[&str] = &["start", "stop", "export", "clear"];
            match rest.get(0).map(|s| *s) {
                Some("start") => Ok(json!({ "id": id, "action": "har_start" })),
                Some("stop") => Ok(json!({ "id": id, "action": "har_stop" })),
                Some("export") => Ok(json!({ "id": id, "action": "har_export" })),
                Some("clear") => Ok(json!({ "id": id, "action": "har_clear" })),
                Some(sub) => Err(ParseError::UnknownSubcommand {
                    subcommand: sub.to_string(),
                    valid_options: VALID,
                }),
                None => Err(ParseError::MissingArguments {
                    ...
                    usage: "har <start|stop|export|clear>",
                }),
            }
        }
...
mod tests {
    ...
    fn test_parse_connect() {
        ...
        let input: Vec<String> = vec!["connect".to_string(), "9222".to_string()];
        let cmd = parse_command(&input, &flags).unwrap();
        assert_eq!(cmd["action"], "launch");
